...
function affichePanier() {
    ...
    fetchPanierData()
        .then((paniers) => {
            ...
            main.innerHTML = TEMPLATE_TICKETS({ paniers: paniers, total: total });
            console.log(paniers);
...
async function fetchPanierData() {
    ...
    let json = await fetch(`${URL_API}/utilisateurs/${userId}/panier`, {
        headers: {
        ...
        .then((response) => {
            if (!response.ok) {
                throw new Error("Erreur de récupération des données du panier");
            }
